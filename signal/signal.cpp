...
/**
    ...
    (6) int sigprocmask(int how, const sigset_t *set, sigset_t *oldset);   用来屏蔽信号、解除屏蔽也使用该函数。其本质,读取或修改进程的信号屏蔽字(PCB 中) 成功:0;失败:-1
    (7) int sigpending(sigset_t *set);         获取未决信号集 成功:0;失败:-1
    (8) int sigsuspend(const sigset_t *mask);         临时解除对信号的屏蔽，然后进入休眠状态，直到捕捉到一个信号 成功:-1;失败:不返回
    (9) int sigaction(int signum, const struct sigaction *act, struct sigaction *oldact);    信号处理函数的设置和获取 成功:0;失败:-1
    (10) int sigwait(const sigset_t *set, int *signo);         用于等待一个信号的产生 成功:0;失败:-1
    (11) int sigwaitinfo(const sigset_t *set, siginfo_t *info);    用于等待一个信号的产生 成功:0;失败:-1
    (12) int sigtimedwait(const sigset_t *set, siginfo_t *info, const struct timespec *timeout);    用于等待一个信号的产生 成功:0;失败:-1
    (13) int sigqueue(pid_t pid, int sig, const union sigval value);    向指定进程发送一个信号 成功:0;失败:-1
    (14) int sigsuspend(const sigset_t *mask);         临时解除对信号的屏蔽，然后进入休眠状态，直到捕捉到一个信号 成功:-1;失败:不返回
    (15) int sigwait(const sigset_t *set, int *signo);         用于等待一个信号的产生 成功:0;失败:-1
    (16) int sigwaitinfo(const sigset_t *set, siginfo_t *info);    用于等待一个信号的产生 成功:0;失败:-1
    (17) int sigtimedwait(const sigset_t *set, siginfo_t *info, const struct timespec *timeout
 */
void sigset_example() {
    sigset_t set;
...
void system_call(int option) {
    if (option == 1) {
         ...
         *          ITIMER_VIRTUAL: 以进程在用户模式CPU消耗的时间递减，计时器到期时，内核向进程发送SIGVTALRM信号
         *          ITIMER_PROF: :以进程在用户模式执行时间和进程背后系统的执行时间, 计时器到期时，内核向进程发送SIGPROF信号
         * 
         */
