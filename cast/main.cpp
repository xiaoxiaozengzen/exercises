...
void down_type_cast_ptr() {
    ...
    // Derived &dref3=dynamic_cast<Derived&>(bobj);//方式三，只有Base class中有虚函数才能使用，否则编译错误（Derived class中有虚函数也不行）
...
void const_example() {
    ...
    ref = 20; // 合法，虽然num为const，但通过const_cast修改了引用。这会导致未定义行为，且程序设计有无问题，为什么要修改const
