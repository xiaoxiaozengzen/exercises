...
int main(int argc, char** argv) {
    ...
    // 调用进程到目前为止使用的用户CPU时间
    std::cout << "tms_utime: " << tms_value.tms_utime << " ticks" << ", s: " << double(tms_value.tms_utime)/ticks << std::endl;
    // 调用进程到目前为止使用的系统cpu时间
    std::cout << "tms_stime: " << tms_value.tms_stime << " ticks" << ", s: " << double(tms_value.tms_stime)/ticks << std::endl;
    // 包含等待所有子进程的用户cpu时间
    std::cout << "tms_cutime: " << tms_value.tms_cutime << " ticks" << ", s: " << double(tms_value.tms_cutime)/ticks << std::endl;
    // 包含等待所有子进程的系统cpu时间
    std::cout << "tms_cstime: " << tms_value.tms_cstime << " ticks"<< ", s: " << double(tms_value.tms_cstime)/ticks << std::endl;
    ...
    clocktime = clock();
    std::cout << "clocktime: " << clocktime << ", s: " << double(clocktime) / CLOCKS_PER_SEC << std::endl;
 
    // 5.clock_gettime
    // CLOCK_REALTIME：为实际的时间。可以使用命令（date）或是系统调用去修改。如果使用了NTP， 也会被NTP修改。当系统休眠（suspend）时，仍然会运行的（系统恢复时，kernel去作补偿）。
