...
void BasicSetMemberTypes() {
    ...
    if(std::is_same<std::forward_list<int>::const_iterator, std::_Fwd_list_const_iterator<int>>::value) {
...
void ConFun() {
    ...
    std::cout << "second_1:"; for (int& x: second_1) std::cout << " " << x; std::cout << '\n';
...
void MemFun() {
    ...
    // 1. iterator
    std::forward_list<int>::iterator it = my_list.before_begin(); // 不要解引用，这个是为了emplace_after等接口使用的
    ...
    std::cout << "my_list2:"; for (int& x: my_list2) std::cout << " " << x; std::cout << '\n';
    // 指向迭代器要传输位置之前的元素
    ...
    std::cout << "my_list2:"; for (int& x: my_list2) std::cout << " " << x; std::cout << '\n';
    //  binary_pred(*i,*(i-1)), i start from second, remove i from the forward_list if the predicate returns true.
    ...
    std::cout << "my_list5:"; for (int& x: my_list5) std::cout << " " << x; std::cout << '\n';
    my_list7.merge(my_list8, [](int first, int second) { return (first < second); });
    std::cout << "my_list7:"; for (int& x: my_list7) std::cout << " " << x; std::cout << '\n';
    // 5.5 sort 
    std::forward_list<int> my_list9 = {5, 3, 1, 2, 4};
    my_list9.sort();
    std::cout << "my_list9:"; for (int& x: my_list9) std::cout << " " << x; std::cout << '\n';
