...
void BasicSetMemberTypes() {
  ...
  if (std::is_same<std::unordered_map<double, int>::value_type,
                   std::pair<const double, int>>::value) {
  ...
  if (std::is_same<std::unordered_map<double, int>::allocator_type,
                   std::allocator<std::pair<const double, int>>>::value) {
    std::cout << "allocator_type is std::allocator<std::pair<const double, int>>" << std::endl;
  }
  if (std::is_same<std::unordered_map<double, int>::reference,
                   std::pair<const double, int>&>::value) {
    std::cout << "reference is std::pair<const double, int>&" << std::endl;
  }
  if (std::is_same<std::unordered_map<double, int>::const_reference,
                   const std::pair<const double, int>&>::value) {
    std::cout << "const_reference is const std::pair<const double, int>&" << std::endl;
  }
  if (std::is_same<std::unordered_map<double, int>::pointer,
                   std::pair<const double, int>*>::value) {
    std::cout << "pointer is std::pair<const double, int>*" << std::endl;
  }
  if (std::is_same<std::unordered_map<double, int>::const_pointer,
                   const std::pair<const double, int>*>::value) {
  ...
  if (std::is_same<std::unordered_map<double, int>::iterator,
                   std::iterator<std::input_iterator_tag, std::pair<const double, int>>>::value) {
    std::cout << "iterator is std::iterator<std::input_iterator_tag, std::pair<const double, int>>"
              << std::endl;
  } else {
    std::cout
        << "iterator is not std::iterator<std::input_iterator_tag, std::pair<const double, int>>"
        << std::endl;
  }
  if (std::is_same<
          std::unordered_map<double, int>::const_iterator,
          std::iterator<std::input_iterator_tag, const std::pair<const double, int>>>::value) {
    std::cout << "const_iterator is std::iterator<std::input_iterator_tag, const std::pair<const "
                 "double, int>>"
              << std::endl;
  } else {
    std::cout << "const_iterator is not std::iterator<std::input_iterator_tag, const "
                 "std::pair<const double, int>>"
              << std::endl;
  }
  if (std::is_same<std::unordered_map<double, int>::local_iterator,
                   std::iterator<std::input_iterator_tag, std::pair<const double, int>>>::value) {
    std::cout
        << "local_iterator is std::iterator<std::input_iterator_tag, std::pair<const double, int>>"
        << std::endl;
  } else {
    std::cout << "local_iterator is not std::iterator<std::input_iterator_tag, std::pair<const "
                 "double, int>>"
              << std::endl;
  }
  if (std::is_same<
          std::unordered_map<double, int>::const_local_iterator,
          std::iterator<std::input_iterator_tag, const std::pair<const double, int>>>::value) {
    std::cout << "const_local_iterator is std::iterator<std::input_iterator_tag, const "
                 "std::pair<const double, int>>"
              << std::endl;
  } else {
    std::cout << "const_local_iterator is not std::iterator<std::input_iterator_tag, const "
                 "std::pair<const double, int>>"
              << std::endl;
  }
}
...
void ConFun() {
  ...
  std::unordered_map<std::string, int> m3(
      10, std::hash<std::string>());  // initial bucket count and hash function
  std::unordered_map<std::string, int> m4(
      10, std::hash<std::string>(),
      std::equal_to<std::string>());  // initial bucket count, hash function and key equal function
  ...
  std::unordered_map<std::string, int> m8 = {{"a", 1}, {"b", 2}, {"c", 3}};  // initializer list
}
...
void MemFun() {
  ...
  std::unordered_map<std::string, int>::mapped_type& ref3 =
      m.at("b");  // throw out_of_range exception if not found
  ...
  std::pair<std::unordered_map<std::string, int>::iterator,
            std::unordered_map<std::string, int>::iterator>
      range = m.equal_range("a");
  if (range.first != m.end()) {
    std::cout << "equal_range(\"a\"): " << range.first->first << ", " << range.first->second
              << std::endl;
  } else {
    std::cout << "equal_range(\"a\") not found" << std::endl;
  }
  if (range.second != m.end()) {
    std::cout << "equal_range(\"a\"): " << range.second->first << ", " << range.second->second
              << std::endl;
  } else {
    std::cout << "equal_range(\"a\") not found" << std::endl;
  }
  std::pair<std::unordered_map<std::string, int>::iterator,
            std::unordered_map<std::string, int>::iterator>
      rane2 = m.equal_range("e");
  if (rane2.first != m.end()) {
    std::cout << "equal_range(\"e\"): " << rane2.first->first << ", " << rane2.first->second
              << std::endl;
  } else {
    std::cout << "equal_range(\"e\") not found" << std::endl;
  }
  if (rane2.second != m.end()) {
    std::cout << "equal_range(\"e\"): " << rane2.second->first << ", " << rane2.second->second
              << std::endl;
  } else {
    std::cout << "equal_range(\"e\") not found" << std::endl;
  }
  ...
  if (result.second) {
    std::cout << "emplace(\"d\", 4): " << result.first->first << ", " << result.first->second
              << std::endl;
  } else {
    std::cout << "emplace(\"d\", 4) failed, old value: " << result.first->first << ", "
              << result.first->second << std::endl;
  }
  std::pair<std::unordered_map<std::string, int>::iterator, bool> result2 = m.emplace("e", 5);
  if (result2.second) {
    std::cout << "emplace(\"e\", 5): " << result2.first->first << ", " << result2.first->second
              << std::endl;
  } else {
    std::cout << "emplace(\"e\", 5) failed, old value: " << result2.first->first << ", "
              << result2.first->second << std::endl;
  }
  // 5.2 emplace_hint
  ...
  // 5.3 insert
  std::pair<std::unordered_map<std::string, int>::iterator, bool> result3 =
      m.insert(std::make_pair("g", 7));
  if (result3.second) {
    std::cout << "insert(\"g\", 7): " << result3.first->first << ", " << result3.first->second
              << std::endl;
  } else {
    std::cout << "insert(\"g\", 7) failed, old value: " << result3.first->first << ", "
              << result3.first->second << std::endl;
  }
  std::unordered_map<std::string, int>::iterator it6 =
      m.insert<std::pair<std::string, int>>(m.begin(), std::make_pair("h", 8));
  ...
  std::unordered_map<std::string, int> m2 = {{"i", 9}, {"j", 10}, {"k", 11}};
  m.insert(m2.begin(), m2.end());
  m.insert({{"l", 12}, {"m", 13}, {"n", 14}});
  // 5.4 erase
  ...
  std::cout << "load_factor: " << m.load_factor() << ", max_load_factor: " << m.max_load_factor()
            << std::endl;
  m.rehash(10);
  std::cout << "rehash(10): " << m.bucket_count() << ", load_factor: " << m.load_factor()
            << ", max_load_factor: " << m.max_load_factor() << std::endl;
  m.reserve(20);
  std::cout << "reserve(20): " << m.bucket_count() << ", load_factor: " << m.load_factor()
            << ", max_load_factor: " << m.max_load_factor() << std::endl;
