...
void BasicSetMemberTypes() {
    ...
    if(std::is_same<std::unordered_set<int>::iterator, std::iterator<std::forward_iterator_tag, const int>>::value) {
        std::cout << "std::unordered_set<int>::iterator is std::iterator<std::forward_iterator_tag, const int>" << std::endl;
    } else {
        std::cout << "std::unordered_set<int>::iterator is not std::iterator<std::forward_iterator_tag, const int>" << std::endl;
    }
}
...
void MemFun() {
    ...
    std::pair<std::unordered_set<int>::iterator, std::unordered_set<int>::iterator> range = set_int.equal_range(3);
...
struct MyHash {
    std::size_t operator()(const T &t) const {
        return std::hash<std::string>()(t.a);
    }
};
