...
protected:
    /**
        ...
        如果 ptr=NULL，一定是递归到空树的位置，此时将创建的新结点地址送给ptr，因为ptr是引用，新结点的地址自然送入上述某一个指针域，自动将新结点作为叶结点链入二叉搜索树中了。
    ...
    /**
        ...
        这是一个递归处理。
        例如，在上图中想要删除关键码为 78 的结点，它的左、右子树都不空。在它的右子树中找中序下的第一个结点，其关键码为 81。把它的值填补到被删结点中去，
        下面的问题就是删除关键码为 81 的结点了。这个结点左子树为空，用它的右子女（关键码为 88）代替它的位置就可以了。
     */
    bool Earse(const T e, BSTNode<T>* &ptr)    //删除
    {
        ...
        if(ptr == NULL)
        {
            ...
            if(e < ptr->data) {
                ...
                Earse(e, ptr->right);
            } else if (ptr->left != nullptr && ptr->right != nullptr) { //1.要删除的结点ptr同时有左右子树
